...
TOPIC_ARN_EMAIL = "arn:aws:sns:us-east-1:942334388812:EmailSNSTopic"

# TODO 2: Set ARN for SNS topic for order messages
TOPIC_ARN_ORDER = "arn:aws:sns:us-east-1:942334388812:OrderSNSTopic"
...
def publish_email(sns, topic_arn, email_mesg, email_subj):
    ...
    sns.Topic(topic_arn).publish(Message=email_mesg, Subject=email_subj)
...
def convert_order_to_json(porder):
    ...
    return json.dumps(porder, default=utils.jdefault, indent=4)
...
def publish_order(sns, topic_arn, json_order):
    ...
    sns.Topic(topic_arn).publish(Message=json_order)
