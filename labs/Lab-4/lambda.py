...
def lambda_handler(event, context):
    ...
    if numbers:
        ...
        mn, mx, avg, cnt = min(numbers), max(numbers), sum(numbers)/len(numbers), len(numbers)
        result = "Min: %s Max: %s Average: %s Count: %s" % (mn, mx, avg, cnt)
        ...
        "Records": [
            {
                "s3": {
                    "bucket": {
                        "name": "calculator-input-ruan-gato-py"
                    },
                    "object": {
                        "key": "numbers.txt"
                    }
